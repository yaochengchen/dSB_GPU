...
public:
    CDSB(const Mat& J, int batch_size, int n_iter)
        : J_(J), batch_size_(batch_size), n_iter_(n_iter) {
        // you will implement your own init
    }

    void update();                           // TODO: your pure C++/CUDA implementation
    std::vector<double> calc_energy() const; // TODO: your pure C++/CUDA implementation

    Mat x; // shape: (n_vars, batch_size), filled by your solver
...
int main(int argc, char** argv) {
    ...
    for (int qpname : qpnames) {
        ...
        if (fr.Flag) {
            ...
            CDSB dsb(-fr.Hr, /*batch_size=*/100, /*n_iter=*/10000);
            dsb.update();
