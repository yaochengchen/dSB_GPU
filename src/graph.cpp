...
void graph::from_edge_list(int n, const vector<int> &bv, const vector<int> &ev, const vector<double> &w)
{
    n_nodes = n;
    n_edges = bv.size();
    nodes.resize(n);        
...
void graph::load_graph(string in_file)
{
    ...
    vector<int> bv(m), ev(m);
    vector<double> w(m);
    for (int i = 0; i < m; ++i)
        fi >> bv[i] >> ev[i] >> w[i];
    from_edge_list(n,bv,ev,w);
}
